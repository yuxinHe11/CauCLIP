...
class GroupRandomCrop(object):
    ...
    def __call__(self, img_group):
        ...
        return out_images[0] if len(out_images) == 1 else out_images
...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_group):
        ...
        if len(ret_img_group) == 1:
            return ret_img_group[0]
    ...
    def __call__(self, img_group):
        ...
        if v < self.p:
            try:
                ...
                safe_worker = torchvision.transforms.ColorJitter(
                    brightness=self.brightness,
                    contrast=self.contrast,
                    saturation=self.saturation,
                    hue=0.0  
                )
                ret = [safe_worker(img) for img in img_group]
        else:
            ret = img_group
...
class GroupGaussianBlur(object):
    ...
    def __call__(self, img_group):
        ...
        if random.random() < self.p:
            ...
            ret = [img.filter(ImageFilter.GaussianBlur(sigma)) for img in img_group]
        else:
            ret = img_group
...
class GroupSolarization(object):
    ...
    def __call__(self, img_group):
        ...
        if random.random() < self.p:
            ret = [ImageOps.solarize(img) for img in img_group]
        else:
            ret = img_group
